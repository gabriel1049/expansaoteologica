...
function tpt_enqueue_assets() {
	...
	// CSS do Painel do Aluno (dashboard FRONTEND do Tutor, ex: /painel/).
	// Obs.: is_tutor_dashboard() so vale no wp-admin; o painel do aluno e frontend.
	$tpt_is_dashboard = function_exists( 'tutor_utils' )
		&& ( tutor_utils()->is_tutor_frontend_dashboard() || tutor_utils()->is_tutor_dashboard() );
	if ( $tpt_is_dashboard ) {
		$tpt_dash_deps = array( 'tpt-components' );
		if ( wp_style_is( 'tutor-frontend', 'enqueued' ) || wp_style_is( 'tutor-frontend', 'registered' ) ) {
			$tpt_dash_deps[] = 'tutor-frontend';
		}
		wp_enqueue_style( 'tpt-dashboard', TPT_URI . '/assets/css/dashboard.css', $tpt_dash_deps, TPT_VERSION );
	}
